...
function get_userlist()
{
  $.ajax({
	...
	if(!users)
	{
	  alert("ѕолучены неверные данные");
	}
	else
	{
	  $.each(users, function(){
...
function formatMessage(mess)
{
  ...
  {
    var str = "<li id=\"mess_"+mess.id+"\"><tt>"+mess.time+"</tt> <b onClick=addLogin('"+mess.login+"')>"+mess.nickname+"</b> дл€ <b onClick=addLogin('"+mess.to_login+"')>"+mess.to_nickname+"</b>:"+mess.message+"</li>";  
  }
  return str;
}
...
function getDialogs()
{
  ...
  $.ajax({
	...
	$("#dialogLineHolder UL LI").click(function(){loadDialog($(this).attr("id"))});
      ...
      {
	$("#dialogLineHolder UL").append("ƒиалоги отсутствуют. ƒл€ начала диалога напишите кому либо личное сообщение");
      }
    }
  });  
}
